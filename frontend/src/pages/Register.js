...
export default function Register() {
  ...
  const handleReg = async (e) => {
    ...
    try {
      ...
      // alert("Store Created! Please Login.");
      navigate('/login');
    } catch (err) {
    ...
    <div className="flex flex-col items-center justify-center min-h-[80vh]">
      <div className="glass-panel p-10 rounded-3xl shadow-2xl w-full max-w-md relative overflow-hidden animate-in">
        ...
        <div className="text-center mb-10">
          ...
          <h2 className="text-3xl font-extrabold text-slate-800 tracking-tight">New Admin Setup</h2>
          <p className="text-slate-500 mt-2">Create an account to start tracking</p>
        </div>
        ...
        <div className="mt-8 text-center pt-6 border-t border-slate-100">
          <p className="text-slate-500 text-sm">
            Already have an account? <Link to="/login" className="text-emerald-600 font-bold hover:text-emerald-700 hover:underline transition-all">Login Here</Link>
          </p>
        </div>
      </div>

      <p className="mt-8 text-slate-400 text-xs font-medium">© 2026 GFH Secured Systems</p>
    </div>
  );
}